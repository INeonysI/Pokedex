...
async function carregaNovosPokemons(inicio, quantidade) {
  const novosPokemonsCarregados = await pegaDadosDosPokemons(
    pokemonsFiltrados,
    inicio,
    quantidade,
  );
...
async function pegaDadosDosPokemons(pokemons, inicio, quantidade) {
  ...
  let requisicoes = pokemonsSubconjunto.map((pokemon) =>
    requisitaDadosDeUmPokemon(pokemon.name),
  );
...
const pokemonsDisplay = document.querySelector('.pokemons');

pokemonsDisplay.addEventListener('click', (event) => {
  parentes = event.path;
  parentes.map((parente) => {
    if (parente.classList != undefined) {
      if (parente.classList.contains('pokemon')) {
        console.log('Achei um pokemon');
      }
    }
  });
});
